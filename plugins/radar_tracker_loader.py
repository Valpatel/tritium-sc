...
re-exports RadarTrackerPlugin so it's discoverable without modifying
...
# Ensure the plugins/ directory is on sys.path so the radar_tracker
# package can be imported.
_plugins_dir = str(Path(__file__).resolve().parent)
if _plugins_dir not in sys.path:
    sys.path.insert(0, _plugins_dir)

from radar_tracker.plugin import RadarTrackerPlugin  # noqa: E402, F401
