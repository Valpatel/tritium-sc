...
from amy.comms.meshtastic_bridge import MeshtasticBridge, MeshtasticNode, MESHTASTIC_MAX_TEXT
...
class TestStartStop:
    ...
    def test_start_handles_connection_error(self, bridge):
        ...
        with patch.dict("sys.modules", {
            ...
            with patch("amy.comms.meshtastic_bridge.MeshtasticBridge.start") as mock_start:
...
class TestPositionHandler:
    def test_position_updates_tracker(self, bridge, tracker):
        with patch("amy.tactical.geo.latlng_to_local", return_value=(10.0, 20.0, 0.0)):
            packet = {
                "fromId": "!node1",
                "decoded": {
                    "position": {
    ...
    def test_position_publishes_to_eventbus(self, bridge, event_bus):
        with patch("amy.tactical.geo.latlng_to_local", return_value=(5.0, 15.0, 0.0)):
            packet = {
                "fromId": "!node1",
                "decoded": {
                    "position": {
    ...
    def test_position_updates_node_info(self, bridge):
        ...
        with patch("amy.tactical.geo.latlng_to_local", return_value=(0.0, 0.0, 0.0)):
            packet = {
                "fromId": "!node1",
                "decoded": {
                    "position": {
    ...
    def test_position_uses_integer_coords(self, bridge, tracker):
        ...
        with patch("amy.tactical.geo.latlng_to_local", return_value=(1.0, 2.0, 0.0)):
            packet = {
                "fromId": "!node1",
                "decoded": {
                    "position": {
