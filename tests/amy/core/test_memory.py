...
class TestMemoryInit:
    ...
    def test_empty_file_path_uses_default(self):
        """Passing None defaults to data/amy/memory.json."""
        mem = Memory(path=None)
        assert mem.path.endswith("memory.json")
        assert "data/amy" in mem.path or "data\\amy" in mem.path
...
class TestObservations:
    ...
    def test_get_nearby_observations_returns_recent(self, memory_tmpdir):
        ...
        now = 1_000_000.0
        with patch("amy.brain.memory.time") as mock_time:
