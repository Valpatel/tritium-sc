...
from amy.brain.sensorium import SceneEvent, Sensorium
...
class TestPush:
    ...
    def test_dedup_same_source_same_text_within_5s(self, sensorium: Sensorium):
        ...
        with patch("amy.brain.sensorium.time.monotonic", return_value=base + 2.0):
            sensorium.push("yolo", "1 person detected")
    ...
    def test_no_dedup_after_5s(self, sensorium: Sensorium):
        ...
        with patch("amy.brain.sensorium.time.monotonic", return_value=base + 6.0):
            sensorium.push("yolo", "1 person detected")
    ...
    def test_no_dedup_different_source(self, sensorium: Sensorium):
        ...
        with patch("amy.brain.sensorium.time.monotonic", return_value=base + 1.0):
    ...
    def test_silence_debounce(self, sensorium: Sensorium):
        ...
        with patch("amy.brain.sensorium.time.monotonic", return_value=base + 31.0):
...
class TestNarrative:
    ...
    def test_narrative_time_formatting(self, sensorium: Sensorium):
        ...
        with patch("amy.brain.sensorium.time.monotonic", return_value=base + 60.0):
            sensorium.push("audio", "User spoke")

        # Event 88s later (will be 2s old at read time -> "Now")
        with patch("amy.brain.sensorium.time.monotonic", return_value=base + 88.0):
            sensorium.push("deep", "Room is bright")

        # Read narrative at base + 90 (ages: 90s, 30s, 2s — all within 120s window)
        with patch("amy.brain.sensorium.time.monotonic", return_value=base + 90.0):
    ...
    def test_narrative_excludes_old_events(self):
        ...
        with patch("amy.brain.sensorium.time.monotonic", return_value=base + 61.0):
...
class TestMood:
    ...
    def test_mood_contemplative_with_thoughts(self, sensorium: Sensorium):
        ...
        base = 1000.0
        for i in range(4):
            with patch("amy.brain.sensorium.time.monotonic", return_value=base + i):
    ...
    def test_mood_calm_on_silence(self, sensorium: Sensorium):
        ...
        for i in range(3):
            with patch("amy.brain.sensorium.time.monotonic", return_value=base + i * 31):
...
class TestRecentThoughts:
    ...
    def test_returns_last_eight(self, sensorium: Sensorium):
        ...
        with patch("amy.brain.sensorium.time.monotonic", return_value=base + 20.0):
