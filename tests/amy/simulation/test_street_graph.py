...
def _make_loaded_graph(temp_cache_dir: str) -> StreetGraph:
    ...
    with patch("amy.tactical.street_graph._fetch_roads") as mock_fetch:
...
class TestCachingWorks:
    ...
    def test_cache_saves_and_loads(self, temp_cache_dir):
        ...
        with patch("amy.tactical.street_graph._fetch_roads", side_effect=counting_fetch):
            sg1 = StreetGraph()
...
class TestOfflineFallback:
    ...
    def test_returns_none_on_api_failure(self, temp_cache_dir):
        ...
        with patch("amy.tactical.street_graph._fetch_roads", side_effect=failing_fetch):
            sg = StreetGraph()
            sg.load(REF_LAT, REF_LNG, radius_m=300, cache_dir=temp_cache_dir)
    ...
    def test_returns_none_on_empty_response(self, temp_cache_dir):
        ...
        with patch("amy.tactical.street_graph._fetch_roads", side_effect=empty_fetch):
            sg = StreetGraph()
            sg.load(REF_LAT, REF_LNG, radius_m=300, cache_dir=temp_cache_dir)
