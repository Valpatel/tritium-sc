...
from amy.tactical.target_tracker import TargetTracker, TrackedTarget
...
class TestSimRobotNavigation:
    ...
    def test_rover_moves_toward_waypoint(self):
        ...
        for _ in range(10):
            rover.tick(0.1)
        # After 10 ticks at 0.1s each (1s), should move ~2 units
        assert rover.position[0] > initial_x
        assert abs(rover.position[0] - 2.0) < 0.5  # ~2m in 1s at 2m/s
    ...
    def test_rover_reaches_waypoint(self):
        ...
        # 3 meters at 2 m/s = 1.5s = 15 ticks
        for _ in range(20):
            rover.tick(0.1)
    ...
    def test_heading_updates_during_movement(self):
        ...
        rover.tick(0.1)
        # heading = atan2(dx, dy) in degrees, waypoint is east (dx=10, dy=0)
        # atan2(10, 0) = 90 degrees
        assert abs(rover.heading - 90.0) < 1.0
