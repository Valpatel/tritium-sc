...
from amy.comms.meshtastic_bridge import MeshtasticNode
...
class TestGetDiscover:
    def test_discover_returns_devices(self):
        with patch(
            "amy.comms.meshtastic_bridge.MeshtasticBridge.discover",
    ...
    def test_discover_empty(self):
        with patch("amy.comms.meshtastic_bridge.MeshtasticBridge.discover", return_value=[]):
            client = TestClient(_make_app(bridge=_mock_bridge()))
            resp = client.get("/api/mesh/discover")
            assert resp.status_code == 200
            assert resp.json()["count"] == 0
