"""Unit tests for amy.comms.mqtt_bridge.MQTTBridge."""
...
class TestConnectionCallbacks:
    def test_on_connect_success(self, bridge, event_bus):
        ...
        # High-rate data at QoS 0, status/events/ack/thoughts at QoS 1
        ...
        assert topics["tritium/home/robots/+/thoughts"] == 1
...
class TestPayloadEdgeCases:
    ...
    def test_robot_telemetry_missing_position(self, bridge, tracker):
        ...
        assert len(tracker.sim_updates) == 1
        # Missing position defaults to origin (0, 0)
        assert tracker.sim_updates[0]["position"] == {"x": 0.0, "y": 0.0}

    def test_robot_telemetry_flat_position_keys(self, bridge, tracker):
        """Robot telemetry with flat x/y keys (not nested position dict)."""
        payload = {"x": 15.5, "y": -8.3, "battery": 0.9, "heading": 45.0}
        msg = _make_msg("tritium/home/robots/rover1/telemetry", payload)
        bridge._on_message(None, None, msg)

        assert len(tracker.sim_updates) == 1
        pos = tracker.sim_updates[0]["position"]
        assert pos["x"] == 15.5
        assert pos["y"] == -8.3

    def test_robot_telemetry_nested_position(self, bridge, tracker):
        """Robot telemetry with nested position dict."""
        payload = {"position": {"x": 7.0, "y": 3.0}, "battery": 0.8}
        msg = _make_msg("tritium/home/robots/rover1/telemetry", payload)
        bridge._on_message(None, None, msg)
...
# ===========================================================================
# Robot thought ingestion
# ===========================================================================


class MockSensorium:
    """Lightweight Sensorium stand-in that records push() calls."""

    def __init__(self):
        self.pushed: list[tuple[str, str, float]] = []  # (source, text, importance)

    def push(self, source: str, text: str, importance: float = 0.5,
             source_node: str = "") -> None:
        self.pushed.append((source, text, importance))


@pytest.mark.unit
class TestRobotThoughtSubscription:
    """Verify that _on_connect subscribes to the robot thoughts topic."""

    def test_thoughts_topic_in_subscriptions(self, bridge, event_bus):
        """on_connect should subscribe to robots/+/thoughts."""
        mock_client = MagicMock()
        bridge._on_connect(mock_client, None, {}, 0)
        ...
        topics = {s[0]: s[1] for s in subs}
        assert "tritium/home/robots/+/thoughts" in topics
        # Thoughts are important state — QoS 1
        assert topics["tritium/home/robots/+/thoughts"] == 1

    def test_thoughts_topic_uses_site_id(self, event_bus, tracker):
        """Subscription should use the configured site_id."""
        b = MQTTBridge(event_bus=event_bus, target_tracker=tracker, site_id="hq")
        mock_client = MagicMock()
        b._on_connect(mock_client, None, {}, 0)
        ...
        subs = mock_client.subscribe.call_args[0][0]
        topics = [s[0] for s in subs]
        assert "tritium/hq/robots/+/thoughts" in topics


@pytest.mark.unit
class TestRobotThoughtRouting:
    """Verify that incoming robot thought messages are routed to _on_robot_thought."""

    def test_thought_message_published_to_eventbus(self, bridge, event_bus):
        """Robot thought should be published as 'robot_thought' event on EventBus."""
        payload = {
            "robot_id": "rover1",
            "type": "thought",
            "text": "I see movement near the north fence",
            "think_count": 5,
            "timestamp": 1708300000.0,
        }
        msg = _make_msg("tritium/home/robots/rover1/thoughts", payload)
        bridge._on_message(None, None, msg)

        # Find the robot_thought event
        thought_events = [
            (et, d) for et, d in event_bus.published
            if et == "robot_thought"
        ]
        assert len(thought_events) == 1
        data = thought_events[0][1]
        assert data["robot_id"] == "rover1"
        assert data["text"] == "I see movement near the north fence"
        assert data["think_count"] == 5

    def test_thought_message_increments_counter(self, bridge):
        """Thought messages should increment the received counter."""
        payload = {"text": "Scanning area", "robot_id": "rover1"}
        msg = _make_msg("tritium/home/robots/rover1/thoughts", payload)
        bridge._on_message(None, None, msg)

        assert bridge._messages_received == 1

    def test_thought_message_updates_device_liveness(self, bridge):
        """Thought messages should update device liveness tracking."""
        payload = {"text": "All quiet", "robot_id": "rover1"}
        msg = _make_msg("tritium/home/robots/rover1/thoughts", payload)
        bridge._on_message(None, None, msg)

        assert "rover1" in bridge._device_last_seen

    def test_thought_with_minimal_payload(self, bridge, event_bus):
        """Thought with only text field should still be handled."""
        payload = {"text": "Idle patrol continues"}
        msg = _make_msg("tritium/home/robots/rover2/thoughts", payload)
        bridge._on_message(None, None, msg)
        ...
        assert len(thought_events) == 1
        data = thought_events[0][1]
        assert data["robot_id"] == "rover2"
        assert data["text"] == "Idle patrol continues"

    def test_thought_with_empty_text(self, bridge, event_bus):
        """Thought with empty text should still route but not crash."""
        payload = {"text": "", "robot_id": "rover1"}
        msg = _make_msg("tritium/home/robots/rover1/thoughts", payload)
        bridge._on_message(None, None, msg)
        ...
        assert len(thought_events) == 1
        assert thought_events[0][1]["text"] == ""

    def test_thought_with_missing_text_key(self, bridge, event_bus):
        """Thought payload missing 'text' should use empty string default."""
        payload = {"robot_id": "rover1", "think_count": 3}
        msg = _make_msg("tritium/home/robots/rover1/thoughts", payload)
        bridge._on_message(None, None, msg)
    ...
    def test_thought_robot_id_from_topic_overrides_payload(self, bridge, event_bus):
        """The robot_id should be derived from the topic, not the payload."""
        payload = {"robot_id": "spoofed", "text": "Sneaky thought"}
        msg = _make_msg("tritium/home/robots/real_rover/thoughts", payload)
        bridge._on_message(None, None, msg)
        ...
        assert len(thought_events) == 1
        # Topic-derived robot_id should win
        assert thought_events[0][1]["robot_id"] == "real_rover"


@pytest.mark.unit
class TestRobotThoughtSensorium:
    """Verify that robot thoughts are pushed to the sensorium."""

    def test_thought_pushed_to_sensorium(self, bridge, event_bus):
        """When sensorium is set, robot thoughts should be pushed."""
        sensorium = MockSensorium()
        bridge.set_sensorium(sensorium)

        payload = {"text": "Hostile spotted at north gate", "robot_id": "rover1"}
        msg = _make_msg("tritium/home/robots/rover1/thoughts", payload)
        bridge._on_message(None, None, msg)

        assert len(sensorium.pushed) == 1
        source, text, importance = sensorium.pushed[0]
        assert source == "robot"
        assert "rover1" in text
        assert "Hostile spotted at north gate" in text

    def test_sensorium_not_required(self, bridge, event_bus):
        """Without sensorium, thought should still publish to EventBus."""
        # No set_sensorium() call — sensorium is None
        payload = {"text": "Patrolling sector 7", "robot_id": "rover1"}
        msg = _make_msg("tritium/home/robots/rover1/thoughts", payload)
        bridge._on_message(None, None, msg)
    ...
    def test_sensorium_gets_robot_name_from_payload(self, bridge):
        """If payload has 'name', sensorium text should use it."""
        sensorium = MockSensorium()
        bridge.set_sensorium(sensorium)

        payload = {"text": "Battery low, returning home", "robot_id": "rover1",
                   "name": "Alpha Rover"}
        msg = _make_msg("tritium/home/robots/rover1/thoughts", payload)
        bridge._on_message(None, None, msg)

        assert len(sensorium.pushed) == 1
        _, text, _ = sensorium.pushed[0]
        assert "Alpha Rover" in text
        assert "Battery low, returning home" in text

    def test_sensorium_falls_back_to_robot_id(self, bridge):
        """If no 'name' in payload, sensorium uses the robot_id from topic."""
        sensorium = MockSensorium()
        bridge.set_sensorium(sensorium)

        payload = {"text": "Nothing to report"}
        msg = _make_msg("tritium/home/robots/scout3/thoughts", payload)
        bridge._on_message(None, None, msg)

        _, text, _ = sensorium.pushed[0]
        assert "scout3" in text

    def test_sensorium_exception_does_not_crash(self, bridge, event_bus):
        """If sensorium.push() raises, the thought should still be on EventBus."""
        class BrokenSensorium:
            def push(self, *args, **kwargs):
                raise RuntimeError("sensorium exploded")

        bridge.set_sensorium(BrokenSensorium())

        payload = {"text": "Hello", "robot_id": "rover1"}
        msg = _make_msg("tritium/home/robots/rover1/thoughts", payload)
        bridge._on_message(None, None, msg)

        # EventBus should still have the event
        thought_events = [
            (et, d) for et, d in event_bus.published
            if et == "robot_thought"
        ]
    ...
    def test_set_sensorium_method_exists(self, bridge):
        """Bridge should have a set_sensorium() method."""
        sensorium = MockSensorium()
        bridge.set_sensorium(sensorium)
        assert bridge._sensorium is sensorium

    def test_thought_importance_is_elevated(self, bridge):
        """Robot thoughts should have elevated importance (0.7) in sensorium."""
        sensorium = MockSensorium()
        bridge.set_sensorium(sensorium)

        payload = {"text": "Hostile approaching", "robot_id": "rover1"}
        msg = _make_msg("tritium/home/robots/rover1/thoughts", payload)
        bridge._on_message(None, None, msg)
