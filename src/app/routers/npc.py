...
def _get_simulation_engine(request: Request):
...
@router.get("/thoughts/llm")
async def get_llm_thoughts(request: Request, count: int = 20):
    """Return recent LLM-generated NPC thoughts from NPCThinker.

    These are ambient thoughts generated by a local LLM for neutral NPCs
    during peaceful gameplay.  Requires NPC_THINKING=true env var.
    """
    engine = _get_simulation_engine(request)
    if engine is None:
        raise HTTPException(503, "Simulation engine not available")
    thinker = getattr(engine, "_npc_thinker", None)
    if thinker is None:
        return {"thoughts": [], "enabled": False, "message": "NPC thinking disabled (set NPC_THINKING=true)"}
    thoughts = thinker.get_recent_thoughts(min(count, 100))
    return {"thoughts": thoughts, "enabled": True, "count": len(thoughts)}
