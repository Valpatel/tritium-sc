...
function _addStep(stepData) {
    ...
    if (stepData.result && typeof stepData.result === 'object') {
        ...
        resultHtml = `<div class="step-result mono">${_esc(preview)}</div>`;
    }

    let promptHtml = '';
    if (stepData.prompt) {
        const truncated = stepData.prompt.substring(0, 150);
        promptHtml = `<div class="step-prompt mono">${_esc(truncated)}...</div>`;
    }
...
function _onGenerationComplete(scenario) {
    ...
    if (bonuses.length > 0) {
        bonusHtml = bonuses.map(b =>
            `<div class="bonus-line mono">+ ${_esc(b.name)} (${b.reward} pts)</div>`
        ).join('');
        bonusHtml = `
            <div class="briefing-section">
    ...
    _briefingContainer.innerHTML = `
        ...
        <div class="briefing-section">
            ...
            <div class="briefing-text">${_esc(ctx.reason || 'Unknown threat detected')}</div>
            <div class="briefing-sub mono">Stakes: ${_esc(ctx.stakes || 'Unknown')}</div>
        </div>
        <div class="briefing-section">
            <div class="briefing-label mono">HOSTILE FORCE</div>
            <div class="briefing-text">${_esc(ctx.attacker_name || 'Unknown')} — ${_esc(ctx.attacker_motivation || 'Unknown motives')}</div>
            <div class="briefing-sub mono">Urgency: ${_esc(ctx.urgency || 'medium').toUpperCase()}</div>
        </div>
        <div class="briefing-section">
            <div class="briefing-label mono">CONDITIONS</div>
            <div class="briefing-text">${_esc(weather.weather || 'Standard')} / Visibility: ${_esc(weather.visibility || 'good')}</div>
            ${weather.mood_description ? `<div class="briefing-sub">${_esc(weather.mood_description)}</div>` : ''}
        </div>
        ${waveSummaryHtml}
        <div class="briefing-section">
            <div class="briefing-label mono">VICTORY CONDITION</div>
            <div class="briefing-text">${_esc(wc.victory?.condition || wc.victory?.description || 'Survive all waves')}</div>
        </div>
        ${bonusHtml}
        <div class="briefing-section">
            <div class="briefing-label mono">YOUR FORCES</div>
            <div class="briefing-text">${(scenario.units || []).length} units deployed</div>
            <div class="briefing-sub mono">${(scenario.units || []).map(u => _esc(u.type || u.name)).join(', ')}</div>
        </div>
