...
ELIMINATION_PHRASES: list[str] = [
    "{interceptor} takes down {target}!",
    "{target} ELIMINATED by {interceptor}!",
    "Hostile down! {interceptor} scores the neutralization!",
    "{interceptor} neutralizes {target}!",
    "BOOM! {target} is out!",
    "Nice shot, {interceptor}!",
    "{target} didn't see that coming!",
    "{interceptor} sends {target} back to respawn!",
    "Clean elimination by {interceptor}!",
]

STREAK_PHRASES: dict[int, list[str]] = {
    3:  ["{name} -- ON A STREAK!", "{name} -- TRIPLE NEUTRALIZATION!"],
    5:  ["RAMPAGE! {name} is unstoppable!", "{name} -- MEGA ELIMINATION!"],
    7:  ["DOMINATING! {name} owns the battlefield!", "ULTRA ELIMINATION -- {name}!"],
    10: ["GODLIKE! {name} IS A ONE-UNIT ARMY!", "MONSTER ELIMINATION -- {name}!"],
}

WAVE_START_PHRASES: list[str] = [
    "WAVE {num}: {name}! {count} hostiles incoming!",
    "Here they come! {count} hostiles! Wave {num} -- {name}!",
...
WAVE_COMPLETE_PHRASES: list[str] = [
    "Wave {num} CLEARED! {eliminations} hostiles neutralized!",
    "Area secured! Wave {num} neutralized -- {eliminations} targets down!",
    "All hostiles down! Wave {num} complete -- {eliminations} eliminated!",
    "Outstanding! Wave {num} decimated -- {eliminations} neutralized! Next wave incoming...",
]

VICTORY_PHRASES: list[str] = [
    "VICTORY! NEIGHBORHOOD SECURED! {eliminations} hostiles neutralized across {waves} waves!",
    "ALL WAVES DEFEATED! The neighborhood is SAFE! {waves} waves, {eliminations} neutralized!",
    "MISSION ACCOMPLISHED! {waves} waves crushed! {eliminations} total eliminations!",
    "YOU WIN! {waves} waves cleared! {eliminations} eliminations!",
]

DEFEAT_PHRASES: list[str] = [
    "DEFEAT! The neighborhood has fallen! Survived {waves} waves with {eliminations} neutralizations.",
    "GAME OVER! Hostiles have overrun the defenses! {eliminations} neutralizations weren't enough!",
...
class WarAnnouncer:
    ...
    def __init__(self, event_bus: EventBus, speaker: object | None = None) -> None:
        ...
        self._elimination_count: int = 0
        self._wave_elimination_count: int = 0
    ...
    def elimination_count(self) -> int:
        return self._elimination_count

    @property
    def wave_elimination_count(self) -> int:
        return self._wave_elimination_count
    ...
    def _process_loop(self) -> None:
        ...
        while self._running:
            ...
            try:
                ...
                elif etype == "elimination_streak":
                    self._on_elimination_streak(data)
    ...
    def _on_target_eliminated(self, data: dict) -> None:
        ...
        interceptor = data.get("interceptor_name", data.get("killer_name", "Unknown"))
        target = data.get("target_name", "Unknown")
        self._elimination_count += 1
        self._wave_elimination_count += 1

        text = self._pick_phrase(ELIMINATION_PHRASES, "elimination", interceptor=interceptor, target=target)
        self._announce(text, "elimination", priority=4)

    def _on_elimination_streak(self, data: dict) -> None:
        """Called on elimination streaks (3, 5, 7, 10)."""
        streak = data.get("streak", 0)
        name = data.get("interceptor_name", data.get("killer_name", "Unknown"))

        phrases = STREAK_PHRASES.get(streak)
        if phrases:
            text = self._pick_phrase(phrases, "elimination_streak", name=name)
        else:
            text = f"{name} -- {streak} ELIMINATION STREAK!"
        self._announce(text, "elimination_streak", priority=7)
    ...
    def _on_wave_start(self, data: dict) -> None:
        ...
        self._wave_elimination_count = 0
    ...
    def _on_wave_complete(self, data: dict) -> None:
        ...
        wave_num = data.get("wave_number", 1)
        eliminations = data.get("eliminations", self._wave_elimination_count)

        text = self._pick_phrase(
            WAVE_COMPLETE_PHRASES, "wave",
            num=str(wave_num), eliminations=str(eliminations),
        )
        self._announce(text, "wave", priority=6)
    ...
    def _on_game_over(self, data: dict) -> None:
        ...
        eliminations = data.get("total_eliminations", self._elimination_count)
        waves = data.get("waves_completed", 0)

        if result == "victory":
            text = self._pick_phrase(
                VICTORY_PHRASES, "victory",
                eliminations=str(eliminations), waves=str(waves),
            )
    ...
    def situational_update(self, hostile_count: int, friendly_count: int) -> None:
        ...
        elif hostile_count == 0 and self._wave_elimination_count > 0:
