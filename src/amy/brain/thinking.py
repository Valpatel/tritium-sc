...
Respond with a Lua function call. For complex situations, you may chain 2-3 actions:
    ...
    battle_cry("Contact north!")
Usually ONE action is sufficient. Available actions:
...
class ThinkingThread:
    ...
    def _think_cycle(self) -> None:
        ...
        # Try multi-action parsing first (compound behaviors)
        multi_calls = extract_multi_actions(response_text)
        if len(multi_calls) > 1:
            actions = parse_multi_actions(response_text)
            errors = validate_action_sequence(actions)
            if not errors:
                self._think_count += 1
                valid_actions = [a for a in actions if a.valid]
                if valid_actions:
                    print(f"  [thinking: {len(valid_actions)} actions] ({dt:.1f}s)")
                    for a in valid_actions:
                        formatted = format_motor_output(a)
                        if a.action == "think":
                            print(f"    [think]: {a.params[0]}")
                        elif a.action == "say":
                            print(f"    [thinking->say]: {a.params[0]}")
                        else:
                            print(f"    [thinking->{formatted}]")
                        self._dispatch(a)
                    return
            else:
                print(f"  [multi-action rejected: {'; '.join(errors)}]")
                # Fall through to single-action parse

        # Single action (most common path)
    ...
    def _dispatch(self, result) -> None:
        ...
        if result.action == "say":
            ...
            node = commander.primary_camera
            if node is not None and node.has_ptz:
                from ..actions.motor import auto_track
            ...
            if node is not None and node.has_ptz and commander.motor is not None:
                from ..actions.motor import survey_room
            ...
            from ..actions.motor import head_shake
            node = commander.primary_camera
            if node is not None and node.has_ptz and commander.motor is not None:
            ...
            from ..actions.motor import double_take
            node = commander.primary_camera
            if node is not None and node.has_ptz and commander.motor is not None:
            ...
            from ..actions.motor import curious_tilt
    ...
    def _handle_look_at(self, direction: str) -> None:
        ...
        if direction == "person":
            if commander.vision_thread and commander.vision_thread.person_target:
                from ..actions.motor import track_person
                commander.motor.set_program(
