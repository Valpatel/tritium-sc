...
class MQTTBridge:
    ...
    MQTT Topic Hierarchy:
        ...
        tritium/{site}/robots/{robot_id}/thoughts     <- Robot LLM thinking stream
    ...
    def __init__(
        ...
        # Auto-discovered MQTT robot nodes: robot_id -> MQTTSensorNode
        self._robot_nodes: dict[str, "MQTTSensorNode"] = {}
        # Commander reference for registering auto-discovered nodes
    ...
    def set_commander(self, commander) -> None:
        """Set commander reference for auto-registering discovered robot nodes."""
        self._commander = commander

    def set_sensorium(self, sensorium) -> None:
        """Set sensorium reference for pushing robot thoughts into Amy's awareness."""
    ...
    def publish(self, topic: str, payload: str, qos: int = 0, retain: bool = False) -> None:
        ...
        Used by MQTTSensorNode to send commands. For internal use,
        prefer the typed publish_* methods which accept dicts.
        """
        ...
        try:
            if category == "cameras":
                ...
                if action == "telemetry":
                    ...
                    self._on_robot_thought(device_id, payload)
                elif action == "status":
    ...
    def _on_robot_telemetry(self, robot_id: str, payload: dict) -> None:
        ...
        Auto-discovers new robots: first telemetry from an unknown robot_id
        creates an MQTTSensorNode and registers it with Commander.
        """
        # Auto-discover new robot nodes
        if robot_id not in self._robot_nodes:
            try:
                from amy.nodes.mqtt_robot import MQTTSensorNode
                node = MQTTSensorNode(self, robot_id, site_id=self._site)
                self._robot_nodes[robot_id] = node
                # Register with Commander if available
                if self._commander is not None and hasattr(self._commander, "nodes"):
                    self._commander.nodes[node.node_id] = node
                logger.info(f"Auto-discovered MQTT robot: {robot_id}")
            except Exception as e:
                logger.debug(f"Failed to create MQTTSensorNode for {robot_id}: {e}")

        # Update node telemetry if we have a node for this robot
        node = self._robot_nodes.get(robot_id)
        if node is not None:
            node.update_telemetry(payload)

        # Robots may publish position as flat x/y keys or nested {"position": {"x":..., "y":...}}
        position = payload.get("position", None)
        if position is None or not isinstance(position, dict):
            position = {"x": payload.get("x", 0.0), "y": payload.get("y", 0.0)}
    ...
    def _on_robot_thought(self, robot_id: str, payload: dict) -> None:
        """Robot published a thought -> push to Sensorium + EventBus.

        Creates the bidirectional intelligence loop:
        Robot detects -> Robot thinks (LLM) -> publishes thought via MQTT ->
        Amy receives here -> Amy reasons about fleet via Sensorium + EventBus.
        """
        thought_text = payload.get("text", "")
        robot_name = payload.get("name", robot_id)

        # Push to sensorium so Amy's thinking thread sees robot cognition
        if self._sensorium is not None:
            try:
                self._sensorium.push(
                    "robot",
                    f"Robot {robot_name}: {thought_text}",
                    importance=0.7,
                )
            except Exception as e:
                logger.debug(f"Sensorium push failed for robot thought: {e}")

        # Publish to EventBus for UI/WebSocket consumers
        self._event_bus.publish("robot_thought", {
            **payload,
            "robot_id": robot_id,
            "text": thought_text,
        })
